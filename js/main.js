...
function loadState() {
  ...
  function wireTheme() {
    ...
    try { localStorage.removeItem("vms_theme"); } catch {}
  ...
  function removeThemeToggles() {
    document.querySelectorAll('#themeSwitch, [data-theme-toggle], .theme-toggle, .dark-mode-toggle, .light-mode-toggle').forEach(el => el.remove());
    document.documentElement.removeAttribute("data-theme");
  }
...
function wireAdminUsers() {
  ...
  if (!table || !search || !btnCreate || !modalEl) return;
      ...
      <tr>
        ...
        <td>
          <div class="d-flex gap-2 flex-wrap">
            <button class="btn btn-sm admin-edit-btn" data-action="edit" data-email="${encodeURIComponent(u.email)}">
              <i class="bi bi-pencil-square me-1"></i> Edit
            </button>
            <button class="btn btn-sm btn-outline-danger rounded-3 px-3" data-action="delete" data-email="${encodeURIComponent(u.email)}">
